...
const PagesAdminHome = () => {
  ...
  const { data: eduBalance, refetch: refetchEdu, isLoading: isLoadingEdu } = useBalance({
    address: isConnected ? (address as `0x${string}`) : undefined,
    token: EDU_CONTRACT_ADDRESS as `0x${string}`,
  });

  const { data: phiiBalance, refetch: refetchPhii, isLoading: isLoadingPhii } = useBalance({
    address: isConnected ? (address as `0x${string}`) : undefined,
    token: PHII_CONTRACT_ADDRESS as `0x${string}`,
  });
    ...
    <div className="min-h-screen bg-[#f9140D] text-white font-sans relative overflow-hidden px-6 md:px-16">
      <div className="bg-white rounded-xl shadow-lg max-w-3xl mx-auto mt-24 hover:shadow-red-500/40 transition-shadow duration-300 overflow-hidden relative">
        <div className="relative w-full h-48 sm:h-64 flex">
          <Image src={MancerBanner} alt="Mancer Banner" fill className="object-cover w-full h-full" />
